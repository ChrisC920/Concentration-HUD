...
description: Use when setting up the Meta Wearables DAT SDK in an iOS app — Swift Package Manager integration, Info.plist configuration, Wearables.configure() initialization, URL callback handling, and the first connection to Ray-Ban Meta or Meta Ray-Ban Display glasses.
---
...
- See the `camera-streaming` skill — Resolution, frame rate, photo capture
- See the `mockdevice-testing` skill — Test without hardware
- See the `session-lifecycle` skill — Handle pause/resume/stop
- See the `permissions-registration` skill — Camera permission flows
